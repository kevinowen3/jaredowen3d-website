...
Two copies of the site are still live and indexable and are *not* covered by that redirect: the GitHub Pages preview and the Worker's own `.workers.dev` URL. See todos.
...
- **2026-09-09** — **jaredowenanimations.com reduced to a 301 onto jaredowen3d.com**, ending the duplicate-site problem for that hostname. A redirect beats `rel="canonical"` here because canonical is only a hint to search engines, whereas a redirect means there genuinely is one site and old links land in the right place. The recipe, in order — the ordering matters:
  1. Worker → **Domains** tab → remove `jaredowenanimations.com` and `www.`. **This has to come first:** while a hostname is attached as a Worker custom domain, the Worker answers it and a zone redirect rule never gets a look in. Removing them also deletes the zone's DNS records, so the domain goes dark until step 3.
