...
├── docs/
│   └── web-presence-research.md   2026-09-12 sweep of every page that references Jared or his videos — press, podcasts, Wikipedia citations, profiles, non-English coverage. Check here before adding press links or social icons
...
- [ ] **Footer Facebook link points at the smaller of Jared's two pages.** All four pages link `facebook.com/JaredOwen3d` (~117K followers); `facebook.com/jaredowenanimations` (~975K) is the one the Shorts channel, the old Wix site and the social-media contractor use. Confirm with Jared which is primary, then switch. Details and other unlinked profiles (Shorts channel, Threads, LinkedIn) in `docs/web-presence-research.md`.
...
- **2026-09-09** — **The store is [Fourthwall](https://fourthwall.com/)**, not Shopify (an earlier note in this file guessed Shopify from the Rails-ish response headers — wrong). It is in Fourthwall's pre-launch state: `store.jaredowen3d.com/` 302s to `/password`, which renders a public **"Coming soon" page**. That is what *every* visitor sees, signed in or not — so the new Store tab currently leads to that holding page until Jared publishes the store in Fourthwall. The `/password` path is just Fourthwall's fixed URL for the unpublished-store holding page; no password is set.
- **2026-09-12** — **Mailing-list form removed from the Fourthwall coming-soon page.** It shipped on by default and was collecting addresses Jared had no plan to email. Turned off in the Fourthwall admin (Site design → Layout → page dropdown "Coming soon" → uncheck *Show mailing list form* → Save) and verified the live page no longer has an email input. Fourthwall never emails subscribers itself; it only stores them (CSV under Settings → General → Email marketing, visible once at least one exists). Nothing in this repo changed — the Store tab still points at the same URL.
...
- **2026-09-12** — **Web-presence research and a fifth press item.** A wide web sweep for anything referencing Jared or his videos is written up in `docs/web-presence-research.md` (press, podcasts, Wikipedia citations, a peer-reviewed paper, Epic, non-English coverage, profile pages, and what was checked and not found). From it, the diginomica interview by George Lawton (2024-04-09, "Telling 3D stories about things") was added to the About page press list, placed chronologically between Hackaday and vidIQ. The rest of the findings are candidates only; the Facebook-page discrepancy is tracked in the todos.
